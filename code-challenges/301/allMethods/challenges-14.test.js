...
const toTitleCase = (arr) => {
  // console.log(arr.map(el => el.charAt(0).toUpperCase()));
  // console.log(arr.map(el => el.substring(1)));
  return arr.map(el => `${el.charAt(0).toUpperCase()}${el.substring(1)}`);
};
...
let biggerThanLuke = (arr) => {
  let emptyStr = '';
  if(arr.length > 0){
    let luke = arr.filter(el => el.name.includes('Luke'));
    let lukeMass = parseInt(luke[0].mass);
    // console.log(lukeMass);
    let biggerChar = arr.filter(el => parseInt(el.mass) > lukeMass);
    return biggerChar.map(el => el.name).join(' - ');
  } else{
    return emptyStr;
  }
}
...
------------------------------------------------------------------------------------------------ */
const isSecure = (url) => {
  // let safeCheck = url.substring(0,9);
  // if(safeCheck === 'https://'){
  //   return true
  // } else{
  //   return false
