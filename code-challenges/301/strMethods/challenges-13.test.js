...
const standardizePhoneNumbers = (arr) => {
  // console.log(`${arr[0].substring(1,4)}${arr[0].substring(6,9)}${arr[0].substring(10,14)}`);
...
const onlyOddChars = (str) => {
  ...
  return newStr.filter((element, index) => index % 2 === 1).join('');
};
...
const allHappy = (arr) => {
  // console.log(arr.map(el => el.includes(':)')));
...
const characterByIndex = (arr) => {
  ...
  //   resArr.push(arr[i].charAt(i));
  //   // let str = arr[i];
  //   // resArr.push(str.charAt(i));
  // }
  // return resArr;
};
